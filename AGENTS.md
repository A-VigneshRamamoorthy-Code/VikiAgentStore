...
The page is JavaScript-rendered, so `curl` alone proves nothing. Headless Chrome
was found to **hang** on this project's macOS setup — use **jsdom** instead and
execute the real `app.js`:

```js
const dom = new JSDOM(html, { runScripts: 'outside-only', url: BASE });
dom.window.fetch = () => Promise.resolve({ ok: true, json: () => Promise.resolve(catalog) });
dom.window.eval(appjs);
dom.window.document.dispatchEvent(new dom.window.Event('DOMContentLoaded'));
```

Then assert on `document.body.innerHTML`.
...
- DNS for `agentstore.sololeapinc.com` is a **CNAME to `cname.vercel-dns.com`**,
  managed at **Squarespace**. It is already configured.
- The Vercel project (`viki-agent-store`) is **not linked to GitHub**, so it does
  **not** auto-deploy on push. The `publish` workflow deploys it explicitly.
- ⚠️ **If `VERCEL_TOKEN` is not set, the production domain will silently serve
  stale content** while GitHub Pages updates normally. The workflow emits a
  warning and a job summary when this happens, rather than failing.
