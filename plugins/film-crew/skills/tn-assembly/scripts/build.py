...
from config import (Project, parse_approvals, publish_scripts,
                    require_overwrite_approval, say)  # noqa: E402
...
def build_clip(pr, outdir, idx, spec):
    ...
    Sections are produced ahead of time by cut.py, which uses
...
def main():
    ...
    for p in (intro_src, outro_src):
        if not os.path.exists(p):
            raise SystemExit(
                ...
                f"  python3 ../../head-of-marketing/scripts/stings.py {pr.root}")
    ...
    for ep in plan["episodes"]:
        ...
        require_overwrite_approval(final, pr, approvals)
