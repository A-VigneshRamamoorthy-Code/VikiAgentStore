...
Use this path — it is one of the locations `paper` style's `tts.py` searches
automatically:
...
what the rate was calibrated against (see `story-editor/reference/
...
3. **So never assume a timeline survives a provider change.** Re-measure after
   switching. In `paper` style that means re-running `--sheet`, which
...
```bash
~/.cache/film-crew/tts_env/bin/edge-tts \
