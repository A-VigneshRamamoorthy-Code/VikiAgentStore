...
name: screenwriter
description: >
  Researches topics into sourced narration scripts cut to a target runtime:
  fact ledger, two-source claims, contested-number hedges, line-level linting
  and sensitive-subject rules. Use when asked to research, fact-check, write
  a documentary/explainer/history narration, or fit a script to time. Part of
...
This is the screenwriter's job in the film-crew pipeline: truth and structure.
[`paper` style](../production-designer/styles/paper/) turns the words into a video;
