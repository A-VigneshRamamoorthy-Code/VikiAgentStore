...
| [`screenwriter`](skills/screenwriter/) | Researches the topic and writes the script. Every claim carries two independent citations; the linter fails a script with an unsourced spoken fact, an unhedged contested figure, or a word count that misses the runtime. |
...
The director loads a crew skill only when the pipeline reaches its stage, so
nine skills' worth of instructions never sit in the context window at once.

---

## Styles

A **style** is how the film looks — and it is just a folder under
`skills/production-designer/styles/`. Adding one changes no code.
...
```bash
python3 -m pip install -r skills/production-designer/styles/paper/scripts/requirements.txt
...
```bash
cd skills/production-designer/styles/paper
