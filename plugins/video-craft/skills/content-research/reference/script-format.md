...
`[[pause N]]` is the portable form. `paper-explainer` consumes the
...
To **`paper-explainer`**: line ids become `narration.lines[].id`, so beats
...
To **`voiceover`**: strip the ids, the braces and the direction; keep
