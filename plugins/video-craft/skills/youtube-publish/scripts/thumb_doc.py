"""Generate a 1280x720 thumbnail in the paper-explainer documentary style.
...
`subject.fn` names any function in paper-explainer's `illustrations.py`, so a
...
# paper-explainer owns the art. Import it rather than reimplementing, so the
# thumbnail cannot drift away from the film's own look.
_SKILLS = os.path.dirname(os.path.dirname(os.path.dirname(
    os.path.abspath(__file__))))
sys.path.insert(0, os.path.join(_SKILLS, "paper-explainer", "scripts"))
