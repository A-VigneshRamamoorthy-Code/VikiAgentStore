...
1. **Write the lines**, one idea each, and save them for the voiceover skill:
...
2. **Record them** with the [`voiceover`](../../../voiceover/) skill — one clip
   per line, into `vo/`:

   ```bash
   python3 ../../../voiceover/scripts/narrate.py lines.json -o vo/ \
...
3. **Point the storyboard at the clips.** Paths resolve relative to
   ...
   Before the voiceover exists, use `"duration": 3.2` instead of `audio` to
...
everything that follows it. Always re-run `--sheet` after a voiceover change —
