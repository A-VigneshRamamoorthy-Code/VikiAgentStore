...
elsewhere — by the [`voiceover`](../../voiceover/) skill — and handed to the
...
or faster voiceover moves every beat with it automatically — you do not
re-time the board by hand. But it does mean a re-recorded voiceover changes the
total runtime, so re-run `--sheet` after any change to the narration audio.

### Blocking out timing before the voiceover exists
