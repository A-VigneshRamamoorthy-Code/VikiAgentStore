...
points at an audio clip produced by the [`voiceover`](../../voiceover/) skill.
...
| `duration` | Used **instead of** `audio` to reserve silent time before the voiceover exists. The renderer warns |
...
timeline is derived from those measurements. Change the voiceover and every
